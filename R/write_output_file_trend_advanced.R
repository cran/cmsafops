write_output_file_trend_advanced <- function(outfile, force_v4,
                                    vars, vars_data, var_name, var_name2,
                                    grid_vars, grid_vars_data,
                                    sig, sig2, standard_name,
  ...
  ncvar_put(nc_out, vars[[4]], vars_data$result$target_p2)
  ncvar_put(nc_out, vars[[5]], vars_data$result$target2)
  ...
  long_name_1 <- paste0("linear trend in ", var_name, " based on time")
  long_name_2 <- paste0("linear trend in ", var_name, " based on ", var_name2)
  ...
  ncatt_put(nc_out, sig2$name,
            ATTR_NAMES$STANDARD_NAME, sig2$standard_name,
            prec = PRECISIONS_ATT$TEXT)
  ncatt_put(nc_out, sig2$name,
            ATTR_NAMES$LONG_NAME, sig2$long_name,
            prec = PRECISIONS_ATT$TEXT)
  ncatt_put(nc_out, sig2$name,
            "description", sig2$info,
            prec = PRECISIONS_ATT$TEXT)
